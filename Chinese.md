...
*redis-llm*是一个[Redis模块](https://redis.io/topics/modules-intro)，它将大语言模型（LLM）集成到了Redis中。
...
为了解决这些问题，我写了*redis-llm*模块，将大语言模型集成到Redis中，使得我们可以把Redis作为大语言模型的外部存储。你可以将私有数据通过*redis-llm*存储到Redis中，然后让大语言模型基于这些数据来回答问题。你也可以通过*redis-llm*来和大语言模型对话，*redis-llm*会记录并索引你们大对话历史，使得大语言模型可以记住几乎所有的对话内容。
...
如果你安装了docker，那么可以通过以下命令来运行*redis-llm*。
...
Docker容器跑起来后，你可以跳转到[快速入门章节](#快速入门)，查看如何执行*redis-llm*命令。
...
*redis-llm*使用[CMAKE](https://cmake.org)编译。
...
如果你使用C++，那么你可以使用[redis-plus-plus](https://github.com/sewenew/redis-plus-plus)来发送*redis-llm*命令：
...
如果你使用Python，那么你可以使用[redis-py](https://github.com/redis/redis-py) to send *redis-llm*命令：
...
你可以使用*redis-llm*来创建大语言模型应用程序，例如：聊天机器人。以下是一些相关的术语介绍。
...
通常你输入一段提示词（prompt），LLM会返回一个相应的回答（completion）。使用一段好的提示词才能得到一个好的答案。[这里](https://help.openai.com/en/articles/6654000-best-practices-for-prompt-engineering-with-openai-api)有一些构建提示词的最佳实践可供参考。
...
除了硬编码的提示词外，*redis-llm*也支持提示词模版。你可以构建一个包含变量的模版，给变量赋不同的值，得到不同的提示词。
...
我们可以使用*--VARS*选项来为每个变量赋值。这个选项采用JSON格式，JSON的每个key-value为对应的变量赋值。*redis-llm*会将以上模版渲染为以下提示词，然后使用该提示词来向LLM发问。
...
向量数据库用于存储数据，以及数据对应的embedding，即：向量。embedding可以表示数据的语义，具有相似embedding的数据往往在语义上也是相似的。通常，我们采用ANN算法来搜索给定数据的K个可能的最近邻数据。
